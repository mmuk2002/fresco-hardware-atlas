...
SEGMENTS = [
    ...
    ('Measured samples, explicit limits', 'Validation has two layers. Automated tests cover ambiguous codes, reordered columns, missing quantities, page boundaries, unused sets, strikeouts, correction persistence, and evaluation matching. Source transcribed labels cover eighteen development sets with ninety one components, plus a further nine sets and fifty one components from three other projects. These checks matched the annotated component fields, but the samples do not establish ninety percent accuracy over the entire corpus. OCR and unfamiliar revision layouts still need review.'),
    ('Run locally / Review / Export', 'The submission includes setup instructions, a command line extractor, batch corpus processing, the local review application, tests, source annotations, and an audit report. Start the app with python dash m hardware sets serve, then open localhost port eight thousand. The main tradeoff is inspectable, low cost extraction versus heuristics that still need a human review path. The next validation step is a larger independently labeled benchmark with previously unseen template families.'),
]
    ...
    with sync_playwright() as p:
        ...
        for index, ((title,_), duration) in enumerate(zip(SEGMENTS,durations)):
            ...
            if index==2:
                ...
                page.locator('[data-field="notes"]').first.fill('Reviewed against source: quantity not specified.')
